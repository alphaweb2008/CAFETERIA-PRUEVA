...
export function AdminPanel({ onBack }: { onBack: () => void; onLogout?: () => void }) {
  ...
  const [newItem, setNewItem] = useState({
    name: '', description: '', price: 0, category: 'espresso', image: '', popular: false,
  });
  ...
  const handleAddItem = () => {
    ...
    setNewItem({ name: '', description: '', price: 0, category: 'espresso', image: '', popular: false });
  ...
  const categories = [
    ...
    { key: 'especialidad', label: 'Especialidad', icon: '✨' },
    { key: 'frias', label: 'Frías', icon: '🧊' },
    { key: 'te', label: 'Té', icon: '🍵' },
